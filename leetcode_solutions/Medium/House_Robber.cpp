...
public:
    ...
    int rob(vector<int>& nums) {
        ...
        // Now we can even reduce the space complexity 
        // Since we just need to store just 2 previous states.

        if(n == 1) return nums[0];

        int prev2 = nums[0]; // basically answer at dp[0]
        int prev1 = max(prev2, nums[1]); // basically answer at dp[1]

        for(int i=2; i<n; i++){
            int curr = max(nums[i] + prev2, prev1); 
            // choose curr + prev2 or skip curr and only choose prev 1

            prev2 = prev1;
            prev1 = curr;
        }

        // Since last answer will be stored in prev1
        return prev1;
    }
};