class Solution {
public:
    vector<string> neighbors(string gene, vector<string>& bank) {
        vector<string> ans;
        ...
        for (int i = 0; i < n; i++) {
            for (char eachChar : {'A', 'C', 'G', 'T'}) {
                ...
                if (copyStr[i] != eachChar) {
                    ...
                    // Only include in neighbors if it is in the bank
                    if(find(bank.begin(), bank.end(), copyStr) != bank.end())
    ...
    int minMutation(string startGene, string endGene, vector<string>& bank) {
        ...
        while (!q.empty()) {
            ...
            for (string neighborGene : neighbors(nextGene, bank)) {
