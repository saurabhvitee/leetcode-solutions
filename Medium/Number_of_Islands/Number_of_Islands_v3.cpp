...
public:
    ...
    void bfs(int row, int col, vector<vector<char>>& grid) {
        ...
        while (!q.empty()) {
            ...
            for (auto direction : directions) {
                ...
                if (nextRow >= 0 and nextRow < grid.size() and nextCol >= 0 and
                    nextCol < grid[0].size()) {
                    // If it is valid set of combination then only we will
                    // proceed with BFS.
                    if (grid[nextRow][nextCol] == '1') {
                        grid[nextRow][nextCol] = '0';
    ...
    int numIslands(vector<vector<char>>& grid) {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                if (grid[i][j] == '1') {
                    // If this is a valid island then please carefully increase
                    // the isLand number Also mark all its neighbors inplace '0'
                    numberOfIslands++;
                    bfs(i, j, grid);
                }
            }
