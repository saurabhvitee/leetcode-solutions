class Solution {
public:
    void backtrack(int row, int n, vector<string>& curr, vector<vector<string>>& ans, unordered_set<int>& diagonals,
        ...
        for (int col = 0; col < n; col++) {
            ...
            // Obviously row is unique so we need to check in columns,
            // antidiagonals, diagonals

            if (columns.contains(col) or diagonals.contains(currDiagonal) or
            ...
            backtrack(row + 1, n, curr, ans, diagonals, antidiagonals, columns);
    ...
    vector<vector<string>> solveNQueens(int n) {
        ...
        vector<string> curr = vector<string>(n, string(n, '.')); // Initialize empty board
        ...
        // Pop from (first_row, first_col) so next time recurse function will
        // pick (first_row, first_col+1)
        backtrack(0, n, curr, ans, diagonals, antidiagonals, columns);
        return ans;
    }
};